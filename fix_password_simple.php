...
try {
    ...
    if ($user) {
        ...
        if (password_verify($test_password, $user['password'])) {
            ...
            // Fix the password using direct SQL
            echo "<p>Fixing password...</p>";
            $correct_hash = password_hash('admin123', PASSWORD_DEFAULT);
            
            // Use direct SQL query instead of the update method
            $sql = "UPDATE users SET password = ? WHERE email = ?";
            $stmt = $db->getConnection()->prepare($sql);
            $result = $stmt->execute([$correct_hash, 'admin@caps.com']);
            
            if ($result) {
                echo "<p style='color: green;'>✅ Password updated successfully!</p>";
                echo "<p><strong>New Password Hash:</strong> {$correct_hash}</p>";
