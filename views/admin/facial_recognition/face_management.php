...
$message = '';
$message_type = '';
// Add support for success message passed via redirect
if (isset($_GET['message']) && $_GET['message'] !== '') {
    $message = $_GET['message'];
    $message_type = 'success';
}
// Path to face management directory and web base
$faces_dir = realpath(__DIR__ . '/../../..');
if ($faces_dir === false) {
    $faces_dir = __DIR__ . '/../../..';
}
$faces_web_base = '/capstone/';
...
if ($_POST && isset($_POST['action']) && $_POST['action'] === 'register_face') {
    ...
    if (empty($name)) {
        ...
        if (isset($_FILES['face_image']) && $_FILES['face_image']['error'] === UPLOAD_ERR_OK) {
            try {
                // File upload logic
                $upload_dir = $faces_dir . '/uploads/faces/';
                ...
                if (!in_array($_FILES['face_image']['type'], $allowed_types)) {
                    ...
                    if (move_uploaded_file($_FILES['face_image']['tmp_name'], $filepath)) {
                        // Register the face in JSON file
                        $faces_file = $faces_dir . '/faces_data.json';
                        $faces_data = [];
                        
                        if (file_exists($faces_file)) {
                            $faces_data = json_decode(file_get_contents($faces_file), true) ?: [];
                        }
                        ...
                        $faces_data[] = $new_face;
                        file_put_contents($faces_file, json_encode($faces_data, JSON_PRETTY_PRINT));
...
if (isset($_POST['action']) && $_POST['action'] === 'delete_face') {
    ...
    $faces_file = $faces_dir . '/faces_data.json';
    if ($face_id && file_exists($faces_file)) {
        $faces_data = json_decode(file_get_contents($faces_file), true);
        foreach ($faces_data as $key => $face) {
            if ($face['id'] === $face_id) {
                // Delete image file if exists
                if (isset($face['image_path'])) {
                    $rawPath = $face['image_path'];
                    if (strpos($rawPath, '/capstone/') === 0) {
                        // Convert web path to filesystem path
                        $rel = str_replace('/capstone/', '', $rawPath);
                        $imgPath = rtrim($faces_dir, '/\\') . '/' . ltrim($rel, '/\\');
                    } else {
                        $imgPath = rtrim($faces_dir, '/\\') . '/' . ltrim($rawPath, '/\\');
                    }
                    if (file_exists($imgPath)) {
                        unlink($imgPath);
                    }
                }
                unset($faces_data[$key]);
                break;
            }
        }
        file_put_contents($faces_file, json_encode(array_values($faces_data), JSON_PRETTY_PRINT));
        $message = 'Face deleted successfully!';
        $message_type = 'success';
    } else {
        $message = 'Face not found or data file missing.';
        $message_type = 'error';
    }
}

// Test action for debugging
if (isset($_POST['action']) && $_POST['action'] === 'test_delete') {
    $message = 'Test delete action received. POST data: ' . print_r($_POST, true);
    $message_type = 'success';
}

// Get registered faces from JSON file
$faces_file = $faces_dir . '/faces_data.json';
$registered_faces = ['success' => true, 'faces' => [], 'message' => ''];

if (file_exists($faces_file)) {
    try {
        $faces_data = json_decode(file_get_contents($faces_file), true);
        if (is_array($faces_data)) {
            $registered_faces['faces'] = $faces_data;
        } else {
            $registered_faces['faces'] = [];
        }
    } catch (Exception $e) {
        $registered_faces['faces'] = [];
        $registered_faces['message'] = 'Error reading faces data';
    }
} else {
    $registered_faces['faces'] = [];
}
...
<body>
    ...
    <div class="container">
        <a href="/capstone/views/admin/adashboard.php" class="btn btn-back">
        ...
        <div class="content-grid">
            ...
            <div class="card">
                ...
                <div class="faces-grid">
                    <?php
                    if (!empty($registered_faces['faces'])) {
                        foreach ($registered_faces['faces'] as $face) {
                            $raw_image_path = $face['image_path'] ?? '';
                            if (strpos($raw_image_path, '/capstone/') === 0) {
                                $image_path = $raw_image_path; // already a web path
                            } else {
                                $image_path = $faces_web_base . ltrim($raw_image_path, '/');
                            }
                            ?>
                            <div class="face-card">
                                <img src="<?php echo htmlspecialchars($image_path); ?>" 
                                     alt="<?php echo htmlspecialchars($face['name']); ?>" 
                                     class="face-image"
                                     onerror="this.src='data:image/svg+xml;base64,'">
                                <div class="face-info">
                                    <h3><?php echo htmlspecialchars($face['name']); ?></h3>
                                    <p>Registered: <?php echo date('M d, Y H:i', strtotime($face['created_at'])); ?></p>
                                </div>
                                <div class="face-actions">
                                    <button class="btn btn-delete" onclick="confirmDelete('<?php echo htmlspecialchars($face['id']); ?>')">
                        ...
                        <div style="text-align: center; padding: 40px; color: #6b7280;">
                            ...
                            <p>No faces registered yet. Start by registering a new face above.</p>
                        </div>
                        <?php
                    }
                    ?>
